                         ...
                         uint8_t *Output, uint8_t *AuthTAG)
{
  ...
  if(Mode == MODE_ENCRYPT) /* AES encryption */
  {
    ...
    *(uint32_t*)&AuthTAG[0] = CRYP_DataOut();
    *(uint32_t*)&AuthTAG[4] = CRYP_DataOut();
    *(uint32_t*)&AuthTAG[8] = CRYP_DataOut();
    *(uint32_t*)&AuthTAG[12] = CRYP_DataOut();
  }
  /*------------------ AES Decryption ------------------*/
  else /* AES decryption */
  {
                         ...
                         uint8_t *AuthTAG, uint32_t TAGSize)
{
  ...
  uint8_t temptag[16] = {0}; /* temporary TAG (MAC) */
  ...
  */
  ...
  /* CRYP Initialization Vectors */
  AES_CRYP_IVInitStructure.CRYP_IV0Left = (__REV(*(uint32_t*)&ctr[0]));
  AES_CRYP_IVInitStructure.CRYP_IV0Right= (__REV(*(uint32_t*)&ctr[4]));
  AES_CRYP_IVInitStructure.CRYP_IV1Left = (__REV(*(uint32_t*)&ctr[8]));
  AES_CRYP_IVInitStructure.CRYP_IV1Right= (__REV(*(uint32_t*)&ctr[12]));
  ...
  if(Mode == MODE_ENCRYPT) /* AES encryption */
  {
    ...
    CRYP_DataIn((*(uint32_t*)&blockb0[0]));
    CRYP_DataIn((*(uint32_t*)&blockb0[4]));
    CRYP_DataIn((*(uint32_t*)&blockb0[8]));
    CRYP_DataIn((*(uint32_t*)&blockb0[12]));
    ...
    CRYP_DataIn(*(uint32_t*)&ctr[0]);
    CRYP_DataIn(*(uint32_t*)&ctr[4]);
    CRYP_DataIn(*(uint32_t*)&ctr[8]);
    /* Reset bit 0 (after 8-bit swap) is equivalent to reset bit 24 (before 8-bit swap) */
    CRYP_DataIn((*(uint32_t*)&ctr[12]) & 0xfeffffff);
    
    /* Wait until the OFNE flag is reset */
    while(CRYP_GetFlagStatus(CRYP_FLAG_OFNE) == RESET);
    
    /* Read the Auth TAG in the IN FIFO */
    *(uint32_t*)&temptag[0] = CRYP_DataOut();
    *(uint32_t*)&temptag[4] = CRYP_DataOut();
    *(uint32_t*)&temptag[8] = CRYP_DataOut();
    *(uint32_t*)&temptag[12] = CRYP_DataOut();
  }
  /*------------------ AES Decryption ------------------*/
  else /* AES decryption */
  {
  ...
  for(loopcounter = 0; (loopcounter < TAGSize); loopcounter++)
  {
    ...
    AuthTAG[loopcounter] = temptag[loopcounter];
  }
