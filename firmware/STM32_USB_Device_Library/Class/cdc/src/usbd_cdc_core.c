...
static uint8_t  usbd_cdc_Init (void  *pdev, 
                               uint8_t __attribute__((__unused__)) cfgidx)
{
  uint8_t *pbuf;
...
/**
  ...
  * @retval status
  */
static uint8_t  usbd_cdc_EP0_RxReady (void __attribute__((__unused__))  *pdev)
{ 
  if (cdcCmd != NO_CMD)
  {
    /* Process the data */
...
/**
  ...
  * @retval status
  */
static uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t __attribute__((__unused__)) epnum)
{
  uint16_t USB_Tx_ptr;
...
/**
  ...
  */
static uint8_t  *USBD_cdc_GetCfgDesc (uint8_t __attribute__((__unused__)) speed, uint16_t *length)
{
